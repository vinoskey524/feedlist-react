...
*
* @vinoskey524 • Hamet Kevin E. ODOUTAN (Author)
*
*/
...
export type FEED_LIST_PROPS_TYPE<T extends FL_ORIENTATION_TYPE> = T extends 'vertical'
    ...
    ? {
        ...
        */
        ...
        /**
        * It fires once every time the user reach the bottom of the list.
        * 
        * (Sync Function) - [optional]
        */
...
type FL_PRIVATE_APIs_TYPE = {
    sessionId: string,
    scaffoldId: string,
    scaffoldRef: RefObject<HTMLDivElement>,
    scrollOffset: RefObject<{ x: number, y: number }>,
...
type REF_ELEMENT_TYPE = RefObject<HTMLDivElement>;
...
const _sensor_finally_timout_ = 120;
...
/** Get measure */
const getMeasureFunc = (x: { targetRef: REF_ELEMENT_TYPE }): RECT_TYPE => {
    const targetRef = x.targetRef;
    if (!targetRef.current)
        throw new Error(`Invalid target ref!`);
    const rect = targetRef.current.getClientRects()[0];
    const res: RECT_TYPE = {
        x: rect.x,
        y: rect.y,
        width: rect.width,
...
const FeedListWidget = forwardRef(<T extends FL_ORIENTATION_TYPE>(props: FEED_LIST_PROPS_TYPE<T>, ref: any) => {
    ...
    const headerSensorRef = useRef<HTMLDivElement>(null);
    ...
    const footerBottomOffset = useRef(0).current;
    ...
    */
    ...
    /* On layout */
    const onLayoutFunc = (): void => {
        const { x, y, width, height } = getMeasureFunc({ targetRef: scaffoldRef as any });
    ...
    const toggleFooterVisibilityFunc = (): void => {
        if (!hideFooterOnScroll.current) return;
    ...
    const onScrollFunc = (e: any): void => {
        try {
            ...
            const childEl = document.getElementById(`${el.id}_container`)!;
            ...
            const contentSize = { width: childRect.width, height: childRect.height };
            const contentOffset = {
                x: (childRect.left > 0) ? 0 : -(childRect.left),
                y: (childRect.top > 0) ? 0 : -(childRect.top)
            };

            const fx = contentOffset.x;
            const fy = contentOffset.y;
            ...
            switch (orientation) {
                ...
                case 'vertical': {
                    ...
                    // if (hasHeader)
                    //     delayFunc().then(() => {
                    //         const rect = getMeasureFunc({ targetRef: headerSensorRef });
                    ...
                    // if (hasFooter)
                    //     delayFunc().then(() => {
                    //         const rect = getMeasureFunc({ targetRef: footerSensorRef });
            ...
            const updateHookSubFunc = () => {
                delayFunc().then(() => {
                    ...
                    updateHookFunc({
                        ...
                        value: {
                            amount: getScrollAmountFunc({ parentDim: parentSize, contentDim: contentSize, contentOffset }),
    ...
    */
    ...
    const scrollToFunc = (x: FL_SCROLL_TO_TYPE): void => {
        try {
            ...
            switch (typeof to) {
                ...
                case 'number': {
                    if (to === 0)
                        scaffoldRef.current?.scrollTo({ left: 0, top: 0, behavior })
                    else if (to < 0)
                        scaffoldRef.current?.scrollTo({ left: 10_000_000, top: 10_000_000, behavior })
                    else
        ...
        } catch (e: any) { logFunc('Err :: scrollToFunc() =>', e.message) }
    };
    ...
    const getNextSiblingFunc = <T extends boolean>(x: FL_GET_SIBLING_ARG_TYPE<T>): FL_GET_SIBLING_RETURN_TYPE<T> => {
        try {
            ...
            logFunc('Err :: getNextSiblingFunc() =>', e.message);
            return undefined;
        }
    };
    ...
    const getPrevSiblingFunc = <T extends boolean>(x: FL_GET_SIBLING_ARG_TYPE<T>): FL_GET_SIBLING_RETURN_TYPE<T> => {
        try {
            ...
            logFunc('Err :: getPrevSiblingFunc() =>', e.message);
            return undefined;
        }
    };
        ...
        try {
            ...
            for (let k in args) {
                ...
                switch (k) {
                    ...
                    // case 'onFetch': { onFetch.current = val } break;
    ...
    const component = <>
        {orientation === 'horizontal' ?
            <div
                id={wid}
                ref={scaffoldRef}
                className='overflow_x'
                style={style.current as React.CSSProperties}
                onScroll={onScrollFunc as any}
            >
                <>
                    {isListEmpty.current ?
                        <div style={{ width: '100%', height: '100%' }} className='column_center_all'>
                            {!emptyComponent.current ?
                                <div style={{ color: '#607D8B', fontSize: 14 }}>•••</div> :
                                emptyComponent.current
                            }
                        </div>
                        :
                        <div id={`${wid}_container`} className='rel row'>
            ...
            >
                {/* {hasHeader ?
                    <div ref={headerRef}>
                        ...
                        <div ref={headerSensorRef} style={{ width: 0, height: 0 }} />
                    </div> : <></>
                } */}
                ...
                {render.current &&
                    <>
                        {isListEmpty.current ?
                            ...
                            <div
                                id={`${wid}_container`}
                                className={`${shouldWrap ? 'row flex_wrap' : 'column'}`}
                                style={{
                                    width: scaffoldDim.current.width,
                                    justifyContent: (columns.current === 'auto') ? 'space-evenly' : 'normal'
                                }}
                            >
                ...
                {/* {hasFooter ?
                    <div ref={footerRef} style={{ width: '100%', height: 'auto', position: 'absolute', left: 0, bottom: footerBottomOffset, zIndex: 1 }}>
                        <div ref={footerSensorRef} style={{ width: 0, height: 0 }} />
...
const FeedWrapper = forwardRef((props: FEED_WRAPPER_PROPS_ARG_TYPE, ref: any) => {
    ...
    const scaffoldRef = controllerRef.current.scaffoldRef;
    const scaffoldRect = scaffoldRef.current.getBoundingClientRect();
    const windowWidth = useRef(scaffoldRect.width);
    const windowHeight = useRef(scaffoldRect.height);
    ...
    const onMountFunc = () => {
        ...
        /* On resize */
        window.onresize = () => { setDimensionsFunc() };
    ...
    /* On layout */
    const onLayoutFunc = (): void => {
        const { x, y, width, height } = getMeasureFunc({ targetRef: wrapperRef as any });;
    ...
    const setVisibilityFunc = (x: { top: number, left: number, width: number, height: number }): void => {
        try {
            const top = x.top;
            const left = x.left;
            const width = x.width;
    ...
    /* Track visibility */
    const trackVisibilityFunc = (arg?: FW_TRACK_VISIBILITY_ARG_TYPE): void => {
        if (!canTrackVisibility.current || remove.current)
            return;
        canTrackVisibility.current = false; /* Disable visibility tracking */
        delayFunc().then(() => {
            const rect = getMeasureFunc({ targetRef: wrapperRef as any });
            ...
            /* - */
            if (arg?.retracking && isInViewport.current)
    ...
    /* Set dimensions */
    const setDimensionsFunc = (): void => {
        const scaffoldRef = controllerRef.current.scaffoldRef;
        const scaffoldRect = scaffoldRef.current.getBoundingClientRect();
        windowWidth.current = scaffoldRect.width;
        windowHeight.current = scaffoldRect.height;
    };
    ...
    */
    ...
    const component = <>
        ...
        {!remove.current &&
            <div ref={wrapperRef} style={wrapperDim.current} className='rel column_center_all'>
...
const Sensor = forwardRef((props: SENSOR_PROPS_TYPE, ref: any) => {
    ...
    const scaffoldRef = controllerRef.current.scaffoldRef;
    const scaffoldRect = scaffoldRef.current.getBoundingClientRect();
    const windowWidth = scaffoldRect.width;
    const windowHeight = scaffoldRect.bottom;
    ...
    const watchSensorFunc = (): void => {
        ...
        delayFunc().then(() => {
            const rect = getMeasureFunc({ targetRef: sensorRef as any });
            ...
            const canRenderMore = rdm !== shouldRenderFeeds.current;
            if (canRenderMore)
                renderMoreFeedsFunc();
            ...
            */
            canRenderMore ?
                setTimeout(() => { finallyFunc() }, _sensor_finally_timout_) :
                finallyFunc();
    ...
    const component = <div ref={sensorRef} style={{ width: 0, height: 0 }} />;
    return (render.current && component);
});
...
const TopSensor = forwardRef((props: TOP_SENSOR_PROPS_TYPE, ref: any) => {
    ...
    const onMountFunc = () => {
        ...
        /* watch sensor */
        timer.current = setTimeout(() => { watchSensorFunc() }, 800);
    ...
    const renderMoreFeedsFunc = () => {
        if (isRenderingMoreFeeds.current || feeds.current.length === 0)
            return;
        try {
            /* Scroll slightly down, to avoid feeds rendered at top to scroll down automatically */
            const offset = controllerRef.current.scrollOffset.current!;
            const off = (orientation === 'horizontal' ? offset.x : offset.y) + 100;
            controllerRef.current.scrollToFunc({
                to: off,
                animate: false,
            });
...
const Loading = forwardRef((props: { wid?: string, visible: boolean, isLoading: boolean, customComponent?: JSX.Element }, ref: any) => {
    ...
    const component = <>
        {visible.current &&
            <div className='ref row_center_all' style={{ width: '100%', paddingBlock: 10 }}>
                {isLoading.current ?
                    <>
                        {(customComponent.current === undefined) ?
                            <img src={`data:image/gif;base64,${loadingGif}`} style={{ width: 50, height: 50 }} />
                            :
                            <>{customComponent.current}</>
                        }
                    </>
                    :
                    <div style={{ color: '#607D8B', fontSize: 12 }}>•••</div>
                }
            </div>
        }
    </>;
